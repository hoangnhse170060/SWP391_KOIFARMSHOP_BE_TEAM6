...
using SWP391.BLL.Helper;
using SWP391.BLL.Library;
using System.Net;
using System.Text;


namespace SWP391.BLL.Services.PaymentService
{
    public class VnpayService
    {
        ...
        public VnpayService(VnpayPayResponse vnpayPayResponse, IOptions<VnpayConfig> vnpayConfig, VnpayPayRequest vnpayPayRequest, Swp391Context context, IMapper mapper)
        {
        ...
        //hàm này để sắp các string theo thứ tự từ trên xuống theo bảng chữ cái
        ...
        public string CheckPaymentResponse(VnpayPayResponse vnpayPayResponse)
        {
            ...
            if (string.IsNullOrEmpty(_vnpayPayResponse.vnp_TxnRef))
            {
                return "Invalid transaction reference"; // "RspCode":"01"
            }

            if (!int.TryParse(_vnpayPayResponse.vnp_TxnRef, out int paymentId))
            {
                return "Invalid transaction reference format"; // "RspCode":"01"
            }
            ...
            if (payment.RequiredAmount != (_vnpayPayResponse.vnp_Amount / 100))
            {
            ...
            {
                if (_vnpayPayResponse.vnp_ResponseCode == "00" && _vnpayPayResponse.vnp_TransactionStatus == "00")
                {
